...
import com.tapi.a0028speedtest.functions.home.objs.Constance
import com.tapi.a0028speedtest.functions.main.objs.Vector
import com.tapi.connectview.linespeedview.objs.LineChartData
import com.tapi.connectview.linespeedview.views.LineCharView
import com.tapi.connectview.linespeedview.views.TypeLineView
...
class LineDrawer(val lineView: LineCharView) {
    ...
    fun onSizeChange() {
        ...
        if (listTmpUpload.size > 0 && listTmpDownload.size > 0) {
    ...
    fun startDrawLineDownload(rate: Float, percent: Float) {
        ...
        Log.d(TAG, "startDrawLineDownload: isVisible ${lineView.isVisible} isActivated ${lineView.isActivated}")
        lineView.invalidate()
    ...
    fun startDrawLineDownload(listData: List<NetworkRate>) {
        ...
        if (mHeight != 0f && mWidth != 0f) {
            ...
            for (item in listData) {
                val posX = (item.percent) * mWidth
                val point = PointF(posX, item.rate)
                lineChartDataDownload.setMaxYAxis(mHeight)
                lineChartDataDownload.addPoint(point.x, point.y)
            }
            lineView.invalidate()
            ...
            listTmpDownload = ArrayList(listData)
        }
    ...
    fun startDrawLineUpload(listData: List<NetworkRate>) {
        ...
        if (mHeight != 0f && mWidth != 0f) {
            ...
            listTmpUpload = ArrayList(listData)
        }
