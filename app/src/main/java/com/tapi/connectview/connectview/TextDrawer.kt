package com.tapi.connectview.connectview
...
class TextDrawer(val connectView: ConnectView) {
    ...
    private val colorText: Int = ContextCompat.getColor(connectView.context, R.color.colorBlueText)
    ...
    fun drawText(canvas: Canvas) {
        ...
        mPaintText.textSize = dp2px(30f).toFloat()
    ...
    suspend fun startAnim() {
        ...
        while (isRunAnim) {
            for (item in listScale) {
                delay(20)
                ratioScale = item
                connectView.invalidateView()
                startAnimationText(item)
            }
        }
    }

    private fun startAnimationText(ratio: Float) {
        newRatio = ratio
